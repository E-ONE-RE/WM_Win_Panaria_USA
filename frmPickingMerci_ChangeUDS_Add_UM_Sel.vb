...
Public Class frmPickingMerci_ChangeUDS_Add_UM_Sel

    '*****************************************
    'MANAGE OF CODE VERSION
    Private Const CodeClassObjectName As String = "frmPickingMerci_ChangeUDS_Add_UM_Sel"
    ...
    Private Sub cmdNextScreen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdNextScreen.Click
        ...
        Try 'HERE PUT NORMAL EXECUTION CODE
            ...
            If (Me.txtQtaSfusiConfermata.Text <> "") Then
                ...
                If (clsChangeUDS.UDSChange(0).VarianteImballo.PezziPerScatola > 0) And (wkQtaSfusiConfermata >= clsChangeUDS.UDSChange(0).VarianteImballo.PezziPerScatola) Then
            ...
            'PRIMA UDS LETTA ( NEL CASO DELLA ADD E' PALETTA A CUI AGGIUNGO IL MATERIALE )
            ...
            'GIACENZA ORIGINE ( GIACENZA DA CUI PRELEVO LA MERCE )
            ...
            ArrayGiacenze(0).GiacenzaOrigine.QuantitaConfermataOperatore = wkQtaConfermata
            ArrayGiacenze(0).GiacenzaOrigine.UnitaDiMisuraAcquisizione = clsChangeUDS.UDSChange(0).UdmQtaPrelevataInUdMConsegna
            ArrayGiacenze(0).GiacenzaDestinazione.QuantitaConfermataOperatore = wkQtaConfermata
            ArrayGiacenze(0).GiacenzaDestinazione.UnitaDiMisuraAcquisizione = clsChangeUDS.UDSChange(0).UdmQtaPrelevataInUdMConsegna

            '* Modifica Gestione Qtà Pezzi Sfusi
            ArrayGiacenze(0).GiacenzaOrigine.QuantitaConfermataSfusiOperatore = wkQtaSfusiConfermata
            ArrayGiacenze(0).GiacenzaDestinazione.QuantitaConfermataSfusiOperatore = wkQtaSfusiConfermata


            RetCode = clsSapWS.Call_ZWMS_JOB_CHANGE_UDS_ADD(ArrayGiacenze, Default_PickingMerci_ChangeUDS_ADD, clsUser.GetUserDivisionToUse, clsUser.GetUserNumeroMagazzinoToUse, ArrayGiacenze(0).GiacenzaDestinazione.UbicazioneInfo, clsUser.SapWmsUser.LANGUAGE, clsWmsJob.UDSOnWork, WmOtInfo, outPickExecutionOk, SapFunctionError, False)
            If (RetCode <> 0) Then
                ErrDescription = clsAppTranslation.GetSingleParameterValue(1758, "", "Errore ADD") & vbCrLf & clsAppTranslation.GetSingleParameterValue(348, "", "Verificare e riprovare")
                MessageBox.Show(ErrDescription, AppMsgBoxTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
                Exit Sub
            End If
    ...
    Private Sub frmPickingMerci_ChangeUDS_Add_UM_Sel_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
        '**************************************
        'HERE PUT DECLARATION OF LOCAL VARIABLE
        Dim RetCode As Long = 0
    ...
    Private Sub frmPickingMerci_ChangeUDS_Add_UM_Sel_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '**************************************
        'HERE PUT DECLARATION OF LOCAL VARIABLE
        Dim RetCode As Long = 0
...
#End If
            ...
            Me.lblCodiceUM.Text = clsAppTranslation.GetSingleParameterValue(1414, Me.lblCodiceUM.Text, "Codice UDS Origine")
            Me.Text = clsAppTranslation.GetSingleParameterValue(1413, Me.Text, "Cambia - UDS (Agg.)")
            ...
            Me.lblQtaPrevista.Text = clsAppTranslation.GetSingleParameterValue(3, "", "Qtà Prevista")
            Me.lblUDMQuantita.Text = clsAppTranslation.GetSingleParameterValue(1770, "", "Udm Qtà Prevista")
            Me.lblQtaConfermata.Text = clsAppTranslation.GetSingleParameterValue(1771, "", "Qtà Confermata")
            Me.lblQtaPezziConfermata.Text = clsAppTranslation.GetSingleParameterValue(1772, "", "Qtà Pezzi Confermata")
...
#End If
            ...
            Me.lblInfoUDS.Text = clsAppTranslation.GetSingleParameterValue(1792, "", "MASTER KTAG: ") & Trim(Str(clsChangeUDS.UDSChange(0).UnitaMagazzino)) & " - " & clsAppTranslation.GetSingleParameterValue(1794, "", "K-TAG SOURCE CODE: ")
    ...
    Private Function LoadUdsDataEntered() As Long
        ...
        Try 'HERE PUT NORMAL EXECUTION CODE
            ...
            If (clsChangeUDS.UDSChange(0).UnitaMagazzino = Me.txtUM.Text) Then
                ErrDescription = clsAppTranslation.GetSingleParameterValue(1786, "", "L'Unita Magazzino specificata è uguale a quella Master.") & vbCrLf & clsAppTranslation.GetSingleParameterValue(348, "", "Verificare e riprovare")
                MessageBox.Show(ErrDescription, AppMsgBoxTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
                Me.txtUM.Text = ""
                Exit Function
            End If
    ...
    Private Function RefreshDatiMaterialeAttivo() As Long
        ...
        Try 'HERE PUT NORMAL EXECUTION CODE
            ...
            Me.lblInfoUDS.Text = clsAppTranslation.GetSingleParameterValue(1792, "", "MASTER KTAG: ") & Trim(Str(clsChangeUDS.UDSChange(0).UnitaMagazzino)) & " - " & clsAppTranslation.GetSingleParameterValue(1794, "", "K-TAG SOURCE CODE: ") & Trim(Str(clsChangeUDS.UDSChange(1).UnitaMagazzino))
    ...
    Public Function GetSelectedStockInfo(ByRef outSelezioneOk As Boolean) As Long
        ...
        Try 'HERE PUT NORMAL EXECUTION CODE
            ...
            'SE LA GRIGLIA HA DEI DATI E LO USER HA SELEZIONATO UNA GIACENZA ALLORA PRENDO QUESTA UBICAZIONE COME
            If (Not (clsChangeUDS.objDataTableUDSChangeInfo Is Nothing)) Then
                If (clsChangeUDS.objDataTableUDSChangeInfo.Rows.Count > 0) Then
                    ...
                    If Me.DtGridListInfo.CurrentRowIndex >= 0 Then
                        ...
                        WorkDataRow = clsChangeUDS.objDataTableUDSChangeInfo.Rows(Me.DtGridListInfo.CurrentRowIndex)


                        'ASSOCIO ALL'UDS MASTER i DATI DELL'ELEMENTO SELEZIONATO
                        clsChangeUDS.UDSChange(1).CodiceMateriale = clsUtility.GetDataRowItem(WorkDataRow, "Matnr", "")
                        clsChangeUDS.UDSChange(1).Partita = clsUtility.GetDataRowItem(WorkDataRow, "Charg", "")
